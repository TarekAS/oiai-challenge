locals {
  ...
  frontendsvc_secrets_version = "AWSCURRENT"
...
resource "kubernetes_manifest" "external_secret_frontendsvc" {
  manifest = {
    frontendsvcVersion = "external-secrets.io/v1beta1"
    kind               = "ExternalSecret"
    metadata = {
      name        = local.name
      namespace   = var.namespace
      ...
      dataFrom = [
        {
          extract = {
            ...
            version = local.frontendsvc_secrets_version
          }
        }
      ]
    }
...
resource "kubernetes_manifest" "target_group_binding_frontendsvc" {
  manifest = {
    frontendsvcVersion = "elbv2.k8s.aws/v1beta1"
    ...
    spec = {
      ...
      targetGroupARN = aws_lb_target_group.frontendsvc.arn
      targetType     = "ip"
    }
  }
}
...
resource "aws_lb_target_group" "frontendsvc" {
  name        = local.name
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
...
resource "aws_lb_listener_rule" "frontendsvc" {
  listener_arn = data.aws_lb_listener.selected443.arn

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.frontendsvc.arn
  }
